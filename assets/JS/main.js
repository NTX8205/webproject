        ...
        success: function (data) {
          ...
          for (var i = 0; i < dataLength; i++) {
            ...
            if (data.games[i]["category_id"] == data.categories[0]) {
              ...
              //price
              var cardPrice = $("<a>");
              cardPrice.attr("href", data.games[i]["price"]);
              cardPrice.html("<p>地圖連結:</p>");

              //category
              var cardCategory = $("<p>");
              cardCategory.addClass("category");
              cardCategory.html(
                "<b>類別</b><br>" + data.games[i]["category_id"]
              );

              //出現在畫面上
              cardBody.append(textCenter, cardPrice, cardCategory);
              linkImg.append(cardImg);

              cardDiv.append(linkImg, cardBody);
